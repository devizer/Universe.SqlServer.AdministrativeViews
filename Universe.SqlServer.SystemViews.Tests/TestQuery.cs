...
using Universe.SqlServer.SystemViews.SqlDataAccess;
...
public class TestQuery
{
    ...
    public void D_QueryQueryCache(SqlServerRef server)
    {
        ...
        var firstForDebug = rows.FirstOrDefault(x => !string.IsNullOrEmpty(x.QueryPlan));
        if (firstForDebug == null) firstForDebug = rows.FirstOrDefault();
        Console.WriteLine($"{rows.Count()} QUERIES ON SERVER [{server}]. Below is up the first");
    ...
    public void E_Produce_Html_Report(SqlServerRef server)
    {
        ...
        var dumpLogFile = Path.Combine(TestEnvironment.DumpFolder, server.GetSafeFileOnlyName() + ".Log.log");
        Console.WriteLine(e.GetLogsAsString());
        File.WriteAllText(dumpLogFile, e.GetLogsAsString());
